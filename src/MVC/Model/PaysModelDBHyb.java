...
public class PaysModelDBHyb extends DAOPays {
    protected Connection dbConnect;

    public PaysModelDBHyb() {
    ...
    public List<Pilote> listePilotes(Pays pays) {
        ...
        String query = "select * from PilotesInVille  where idPays = ?";
        try (PreparedStatement pstm = dbConnect.prepareStatement(query)) {
            pstm.setInt(1, pays.getId_pays());
            ResultSet rs = pstm.executeQuery();
            while (rs.next()) {
                int idPilote = rs.getInt(2);
                String matricule = rs.getString(3);
                String nom = rs.getString(4);
                String prenom = rs.getString(5);
                LocalDate dateNaiss = rs.getDate(6).toLocalDate();
                Pilote p = new Pilote(idPilote, matricule, nom, prenom, dateNaiss);
                liste.add(p);
            }
            return liste;
        } catch (SQLException e) {
